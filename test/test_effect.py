...
def test_effect_quarters_limit(app_time_patched_1) -> None:
    app = app_time_patched_1
    app.settings.enable_autopilot = False
    ...
    app.effect_handler.clear_qeueues()
    assert len(app.effect_handler.effect_queue) == 0
    app.effect_handler.load_effect_quarters(effect_name=effect_name, limit_quarters=2)
    assert len(app.effect_handler.effect_queue) == 1
    ...
    # count should be around 6 (fps = 30)
    assert active_log[:5] == [True] * 5
    assert 5 <= sum(active_log) <= 7


"""

def test_effect_quarters_limit_pattern(app_time_patched_1) -> None:
    app = app_time_patched_1
    app.settings.enable_autopilot = False
    ...
    assert len(app.effect_handler.effect_queue) == 0
    frames_pattern = FramesPattern(4, (1,))
    app.effect_handler.load_effect_quarters(effect_name=effect_name, limit_quarters=8, frames_pattern=frames_pattern)
    assert len(app.effect_handler.effect_queue) == 1
