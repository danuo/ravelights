...
from ravelights.core.device_settings import DeviceSettings
...
class Device:
    ...
    def get_matrix_processed_int(self) -> ArrayUInt8:
        ...
        brightness = min(self.settings.global_brightness, self.device_settings.device_brightness)
    ...
    def get_effective_timeline_level(self) -> int:
        ...
        """
        if isinstance(self.device_settings.device_manual_timeline_level, int):
            # manual timeline level is defined on device level
            return self.device_settings.device_manual_timeline_level
